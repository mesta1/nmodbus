...
namespace Modbus.UnitTests.IO
{
	...
	public class ModbusTransportFixture
	{
		...
		public void UnicastMessage_TooManyFailingExceptions(Type exceptionType)
		{
			...
			System.Action action = () => transport.UnicastMessage<ReadCoilsInputsResponse>(request);
		...
		public void CreateResponse_SlaveException()
		{
			...
			byte[] frame = { 2, 129, 2 };
			IModbusMessage message = transport.CreateResponse<ReadCoilsInputsResponse>(frame.Concat(Unme.Common.SequenceUtility.ToSequence(ModbusUtility.CalculateLrc(frame))).ToArray());
